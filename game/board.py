...
This module contains a light-weight numpy based board representation that can be
shared by both human players (via the pygame UI) and reinforcement learning
agents.  The design mirrors the classic 2048 rules and exposes utility
functions for computing rewards, detecting terminal states, and exporting a
state representation suitable for neural network consumption.
"""
...
class GameBoard:
    ...
    def step(self, direction: str) -> MoveResult:
        ...
        Returns:
            MoveResult describing whether a move happened and the reward.
        """
    ...
    def to_normalized_state(self) -> np.ndarray:
        """Return log2 normalized board with zeros for empty tiles."""
        with np.errstate(divide="ignore"):
            log_board = np.where(self.grid > 0, np.log2(self.grid), 0.0)
        max_value = log_board.max() or 1.0
        return (log_board / max_value).astype(np.float32)
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def available_directions() -> Iterable[str]:
